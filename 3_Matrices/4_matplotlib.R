...
# ------------------- Matrixes operation ------------------
# As mentioned above we have 5 Matrices:
# - FieldGoalAttempts
# - FieldGoals
# - Games
# - MinutesPlayed
...
# Exploring the Game matrix
Games
# Discove how many games KobeBrytant played in 2012
Games["KobeBryant", "2012"]

# We want to know how many goals for game every player scored
# The operation is we simple, we divide two matrixes
Goals.per.Game <- round(FieldGoals/Games, 1)
Goals.per.Game


# Lets find out how many minutes every player played per game
Minutes.per.Game <- round(MinutesPlayed/Games, 2)
Minutes.per.Game

# We analyze the performance of every player by calculating the field goals on the attempts
Goals.accuracy <- round(FieldGoals/FieldGoalAttempts, 2)
Goals.accuracy

# Let's see how is the goal rate based on the minutes played  
Goals.rate <- round(FieldGoals/MinutesPlayed, 2)
Goals.rate

# ----------------------- VISUALIZATION ------------------------
# We will use matplotlib as visualization library for matrixes
?matplot

# With the matrixes we are using we need to transpose the columns to rows and vice versa
Field.Goals <- t(FieldGoals)

# -------------First visualization
matplot(Field.Goals, type = "b", pch = 15:18, col = c(1:4,6)) 
# The legend will overlap the image
legend("bottomleft", inset = 0.01, legend = Players, col = c(1:4,6), pch = 15:18, horiz = F)


# ------------Plotting the goal accuracy
matplot(Goal.accuracy,type = "b", pch = 15:18, col = c(1:4,6)) 
legend("bottomleft", inset = 0.01, legend = Players, col = c(1:4,6), pch = 15:18, horiz = F)


# ------------- Plotting Salary per Goals
