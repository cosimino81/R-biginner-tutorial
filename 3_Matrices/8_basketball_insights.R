...
# More generic function in order to reed every dataset
myPlot <- function(data, rows = 1:10){
  subset <- data[rows, , drop=F ]
  par(cex=0.6)
  matplot(t(subset), type = "b", pch = 12:19, col = c(1:4,6), lwd = 2) 
  legend("bottomleft", inset = 0.01, legend = Players[rows], col = c(1:4,6), pch = 12:19, horiz = F)
}

# AFTER WE HAVE CREATED THE FUNCTION WE WILL USE IT IN ORDER TO GET INSIGHTS FROM THE DATA
